...
### `lib/journey/flows/KundeAnlegen`
```ts
/**
...
### `lib/journey/flows/ProjektstatusAktualisieren`
```ts
/**
...
### `lib/journey/flows/StundenErfassen`
```ts
/**
...
export interface WizardContextValue {
  ...
  /** A StepNav announces itself while mounted (the shell renders one step at a time, so this is "the current step has a We… */
  registerNav(): () => void;
  /** True while a StepNav is mounted on the current step. */
  hasNav(): boolean;
}
```
