...
export function KundenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: KundenDialogProps) {
  ...
  const SYSTEM_ASSIGNED: string[] = ["anlagedatum"];
  const [showErrors, setShowErrors] = useState(false);
  const REQUIRED_FIELDS = ['kundenname', 'kundentyp', 'email', 'strasse', 'hausnummer', 'plz', 'ort'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'kundenname': (
      <div key="kundenname" className="space-y-1.5">
        ...
        <Input
          id="kundenname"
          placeholder="z. B. Müller & Co. GmbH"
    ...
    'anlagedatum': (
      <div key="anlagedatum" className="space-y-1.5">
        ...
        <DatePicker
          id="anlagedatum"
          placeholder="Wann wurde der Kunde erfasst?"
          mode="date"
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Besonderheiten, Geschäftsbeziehung, Anforderungen..."
