 ...
 *             but leaves the rule to you: `erfassungsmonat` (derived:computed:Monat aus dem Feld Datum ableiten und den passenden Schlüssel setzen (z. B. datum im März → 'maerz')), `erfassungsjahr` (derived:computed:Jahr aus dem Feld Datum ableiten) *
...
export type StundenBuchenFieldKey = 'abrechenbar' | 'berater' | 'datum' | 'leistung' | 'projekt' | 'stunden' | 'taetigkeit';
...
export interface StundenBuchenFlowOptions {
  ...
  compute: {
    erfassungsmonat: (forms: StundenBuchenForms) => unknown;   // derived:computed:Monat aus dem Feld Datum ableiten und den passenden Schlüssel setzen (z. B. datum im März → 'maerz')
    erfassungsjahr: (forms: StundenBuchenForms) => unknown;   // derived:computed:Jahr aus dem Feld Datum ableiten
  };
}
...
export function useStundenBuchenFlow(options: StundenBuchenFlowOptions) {
  ...
  return {
    ...
    reviewStep: STUNDENBUCHEN_REVIEW_STEP,
