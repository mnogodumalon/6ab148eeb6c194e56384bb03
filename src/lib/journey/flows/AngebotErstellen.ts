 ...
 *             but leaves the rule to you: `angebotsjahr` (derived:computed:Aktuelles Kalenderjahr beim Anlegen des Angebots) *
...
export interface AngebotErstellenFlowOptions {
  ...
  compute: {
    angebotsjahr: (forms: AngebotErstellenForms) => unknown;   // derived:computed:Aktuelles Kalenderjahr beim Anlegen des Angebots
  };
}
...
export function useAngebotErstellenFlow(options: AngebotErstellenFlowOptions) {
  ...
  const plan: PlanStep[] = [
    {
      key: 'angebote', entity: 'angebote', form: angebote, primary: true,      values: (): FormValues => ({
  ...
  return {
    ...
    reviewStep: ANGEBOTERSTELLEN_REVIEW_STEP,
