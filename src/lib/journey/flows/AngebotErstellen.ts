 ...
 *
 * Writes `angebote`: asks `projekt`, `berater`, `angebotstyp`, `angebotsjahr`, `zeitrahmen_anfang`, `zeitrahmen_ende`, `dauer`, `kostentyp`, `kostenbetrag`, `beschreibung`; sets `angebotsstatus` itself.
 ...
 *     steps: { projekt: 1, berater: 2, angebotstyp: 3, angebotsjahr: 3, zeitrahmen_anfang: 3, zeitrahmen_ende: 3, dauer: 3, kostentyp: 3, kostenbetrag: 3, beschreibung: 3 },
 ...
 *     <Bound form={flow.forms.angebote} name="angebotsjahr" />
...
export type AngebotErstellenFieldKey = 'angebotsjahr' | 'angebotstyp' | 'berater' | 'beschreibung' | 'dauer' | 'kostenbetrag' | 'kostentyp' | 'projekt' | 'zeitrahmen_anfang' | 'zeitrahmen_ende';
...
const DEFAULT_STEPS: Record<string, number> = {"angebotsjahr": 3, "angebotstyp": 3, "berater": 2, "beschreibung": 3, "dauer": 3, "kostenbetrag": 3, "kostentyp": 3, "projekt": 1, "zeitrahmen_anfang": 3, "zeitrahmen_ende": 3};
...
export function useAngebotErstellenFlow(options: AngebotErstellenFlowOptions = {}) {
  ...
  const angebote = useStepForm('angebote', {
    fields: ["projekt", "berater", "angebotstyp", "angebotsjahr", "zeitrahmen_anfang", "zeitrahmen_ende", "dauer", "kostentyp", "kostenbetrag", "beschreibung"],
    steps: only(steps, ["projekt", "berater", "angebotstyp", "angebotsjahr", "zeitrahmen_anfang", "zeitrahmen_ende", "dauer", "kostentyp", "kostenbetrag", "beschreibung"]) as Record<string, number>,
    // the plan builds a value from these — required here, whatever the app's base view says
    required: { angebotstyp: true, berater: true, beschreibung: true, dauer: true, kostenbetrag: true, kostentyp: true, projekt: true, zeitrahmen_anfang: true, zeitrahmen_ende: true },
    initial: only(options.initial as FormValues | undefined, ["projekt", "berater", "angebotstyp", "angebotsjahr", "zeitrahmen_anfang", "zeitrahmen_ende", "dauer", "kostentyp", "kostenbetrag", "beschreibung"]),
    messages: only(options.messages as Record<string, string> | undefined, ["projekt", "berater", "angebotstyp", "angebotsjahr", "zeitrahmen_anfang", "zeitrahmen_ende", "dauer", "kostentyp", "kostenbetrag", "beschreibung"]),
  });
