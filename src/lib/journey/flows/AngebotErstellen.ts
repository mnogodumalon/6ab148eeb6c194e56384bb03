 ...
 *
 * Writes `angebote`: asks `angebotstyp`, `projekt`, `berater`, `zeitrahmen_anfang`, `zeitrahmen_ende`, `dauer`, `kostentyp`, `kostenbetrag`, `beschreibung`; sets `angebotsstatus` itself.
 ...
 *             but leaves the rule to you: `angebotsjahr` (derived:computed:Aktuelles Jahr (Jahreszahl des Erstellungsdatums)) *
...
export interface AngebotErstellenFlowOptions {
  ...
  compute: {
    angebotsjahr: (forms: AngebotErstellenForms) => unknown;   // derived:computed:Aktuelles Jahr (Jahreszahl des Erstellungsdatums)
  };
}
...
export function useAngebotErstellenFlow(options: AngebotErstellenFlowOptions) {
  ...
  const angebote = useStepForm('angebote', {
    fields: ["angebotstyp", "projekt", "berater", "zeitrahmen_anfang", "zeitrahmen_ende", "dauer", "kostentyp", "kostenbetrag", "beschreibung"],
    steps: only(steps, ["angebotstyp", "projekt", "berater", "zeitrahmen_anfang", "zeitrahmen_ende", "dauer", "kostentyp", "kostenbetrag", "beschreibung"]) as Record<string, number>,
    initial: only(options.initial as FormValues | undefined, ["angebotstyp", "projekt", "berater", "zeitrahmen_anfang", "zeitrahmen_ende", "dauer", "kostentyp", "kostenbetrag", "beschreibung"]),
    messages: only(options.messages as Record<string, string> | undefined, ["angebotstyp", "projekt", "berater", "zeitrahmen_anfang", "zeitrahmen_ende", "dauer", "kostentyp", "kostenbetrag", "beschreibung"]),
  });
