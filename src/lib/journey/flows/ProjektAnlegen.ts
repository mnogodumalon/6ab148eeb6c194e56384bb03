 ...
 *
 * Writes `projekte`: asks `projektart`, `projektstart_monat`, `projektstart_jahr`, `kunde`, `ansprechpartner_kunde`, `projektleitung`; sets `projektstatus` itself.
 ...
 *     steps: { kunde: 1, projektleitung: 2, projektart: 3, projektstart_monat: 3, projektstart_jahr: 3, ansprechpartner_kunde: 3 },
...
export function useProjektAnlegenFlow(options: ProjektAnlegenFlowOptions = {}) {
  ...
  const projekte = useStepForm('projekte', {
    fields: ["projektart", "projektstart_monat", "projektstart_jahr", "kunde", "ansprechpartner_kunde", "projektleitung"],
    steps: only(steps, ["projektart", "projektstart_monat", "projektstart_jahr", "kunde", "ansprechpartner_kunde", "projektleitung"]) as Record<string, number>,
    ...
    initial: only(options.initial as FormValues | undefined, ["projektart", "projektstart_monat", "projektstart_jahr", "kunde", "ansprechpartner_kunde", "projektleitung"]),
    messages: only(options.messages as Record<string, string> | undefined, ["projektart", "projektstart_monat", "projektstart_jahr", "kunde", "ansprechpartner_kunde", "projektleitung"]),
  });
  ...
  const plan: PlanStep[] = [
    {
      key: 'projekte', entity: 'projekte', form: projekte, primary: true,      values: (): FormValues => ({
  ...
  return {
    ...
    reviewStep: PROJEKTANLEGEN_REVIEW_STEP,
