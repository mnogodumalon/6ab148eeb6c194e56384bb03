/**
 * useZeitBuchenFlow — the plumbing of the flow « Zeit buchen », generated from the plan.
 *
 * Writes `zeiterfassung`: asks `berater`, `projekt`, `leistung`, `stunden`, `erfassungsmonat`, `erfassungsjahr`, `abrechenbar`, `taetigkeit`; sets `datum` itself.
 ...
 *     steps: { berater: 1, projekt: 2, leistung: 3, stunden: 4, erfassungsmonat: 4, erfassungsjahr: 4, abrechenbar: 4, taetigkeit: 4 },
...
export type ZeitBuchenFieldKey = 'abrechenbar' | 'berater' | 'erfassungsjahr' | 'erfassungsmonat' | 'leistung' | 'projekt' | 'stunden' | 'taetigkeit';
...
export function useZeitBuchenFlow(options: ZeitBuchenFlowOptions = {}) {
  ...
  const zeiterfassung = useStepForm('zeiterfassung', {
    fields: ["berater", "projekt", "leistung", "stunden", "erfassungsmonat", "erfassungsjahr", "abrechenbar", "taetigkeit"],
    steps: only(steps, ["berater", "projekt", "leistung", "stunden", "erfassungsmonat", "erfassungsjahr", "abrechenbar", "taetigkeit"]) as Record<string, number>,
    initial: only(options.initial as FormValues | undefined, ["berater", "projekt", "leistung", "stunden", "erfassungsmonat", "erfassungsjahr", "abrechenbar", "taetigkeit"]),
    messages: only(options.messages as Record<string, string> | undefined, ["berater", "projekt", "leistung", "stunden", "erfassungsmonat", "erfassungsjahr", "abrechenbar", "taetigkeit"]),
  });
