 ...
 *
 * Writes `rechnungen`: asks `kunde`, `projekt`, `berater`, `zeiterfassungseintraege`, `rechnungsdatum`, `nettobetrag`, `mehrwertsteuer`, `rechnungsmonat`, `rechnungsjahr`, `notizen`; sets `rechnungsstatus` itself.
 ...
 *     steps: { kunde: 1, projekt: 2, berater: 3, zeiterfassungseintraege: 4, rechnungsdatum: 5, nettobetrag: 5, mehrwertsteuer: 5, rechnungsmonat: 5, rechnungsjahr: 5, notizen: 5 },
...
export type RechnungErstellenFieldKey = 'berater' | 'kunde' | 'mehrwertsteuer' | 'nettobetrag' | 'notizen' | 'projekt' | 'rechnungsdatum' | 'rechnungsjahr' | 'rechnungsmonat' | 'zeiterfassungseintraege';
...
const DEFAULT_STEPS: Record<string, number> = {"berater": 3, "kunde": 1, "mehrwertsteuer": 5, "nettobetrag": 5, "notizen": 5, "projekt": 2, "rechnungsdatum": 5, "rechnungsjahr": 5, "rechnungsmonat": 5, "zeiterfassungseintraege": 4};
...
export function useRechnungErstellenFlow(options: RechnungErstellenFlowOptions = {}) {
  ...
  const rechnungen = useStepForm('rechnungen', {
    fields: ["kunde", "projekt", "berater", "zeiterfassungseintraege", "rechnungsdatum", "nettobetrag", "mehrwertsteuer", "rechnungsmonat", "rechnungsjahr", "notizen"],
    steps: only(steps, ["kunde", "projekt", "berater", "zeiterfassungseintraege", "rechnungsdatum", "nettobetrag", "mehrwertsteuer", "rechnungsmonat", "rechnungsjahr", "notizen"]) as Record<string, number>,
    // the plan builds a value from these — required here, whatever the app's base view says
    required: { mehrwertsteuer: true, nettobetrag: true, rechnungsdatum: true, rechnungsjahr: true },
    initial: only(options.initial as FormValues | undefined, ["kunde", "projekt", "berater", "zeiterfassungseintraege", "rechnungsdatum", "nettobetrag", "mehrwertsteuer", "rechnungsmonat", "rechnungsjahr", "notizen"]),
    messages: only(options.messages as Record<string, string> | undefined, ["kunde", "projekt", "berater", "zeiterfassungseintraege", "rechnungsdatum", "nettobetrag", "mehrwertsteuer", "rechnungsmonat", "rechnungsjahr", "notizen"]),
  });
