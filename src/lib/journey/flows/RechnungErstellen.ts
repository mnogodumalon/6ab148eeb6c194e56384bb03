 ...
 *
 * Writes `rechnungen`: asks `kunde`, `projekt`, `berater`, `zeiterfassungseintraege`, `rechnungsdatum`, `faelligkeitsdatum`, `rechnungsmonat`, `rechnungsjahr`, `nettobetrag`, `mehrwertsteuer`, `notizen`, `gesamtbetrag`; sets `rechnungsstatus` itself.
 ...
 *     steps: { kunde: 1, projekt: 2, berater: 3, zeiterfassungseintraege: 4, rechnungsdatum: 5, faelligkeitsdatum: 5, rechnungsmonat: 5, rechnungsjahr: 5, nettobetrag: 5, mehrwertsteuer: 5, notizen: 5, gesamtbetrag: 5 },
...
export type RechnungErstellenFieldKey = 'berater' | 'faelligkeitsdatum' | 'gesamtbetrag' | 'kunde' | 'mehrwertsteuer' | 'nettobetrag' | 'notizen' | 'projekt' | 'rechnungsdatum' | 'rechnungsjahr' | 'rechnungsmonat' | 'zeiterfassungseintraege';
...
const DEFAULT_STEPS: Record<string, number> = {"berater": 3, "faelligkeitsdatum": 5, "gesamtbetrag": 5, "kunde": 1, "mehrwertsteuer": 5, "nettobetrag": 5, "notizen": 5, "projekt": 2, "rechnungsdatum": 5, "rechnungsjahr": 5, "rechnungsmonat": 5, "zeiterfassungseintraege": 4};
...
export function useRechnungErstellenFlow(options: RechnungErstellenFlowOptions = {}) {
  ...
  const rechnungen = useStepForm('rechnungen', {
    fields: ["kunde", "projekt", "berater", "zeiterfassungseintraege", "rechnungsdatum", "faelligkeitsdatum", "rechnungsmonat", "rechnungsjahr", "nettobetrag", "mehrwertsteuer", "notizen", "gesamtbetrag"],
    steps: only(steps, ["kunde", "projekt", "berater", "zeiterfassungseintraege", "rechnungsdatum", "faelligkeitsdatum", "rechnungsmonat", "rechnungsjahr", "nettobetrag", "mehrwertsteuer", "notizen", "gesamtbetrag"]) as Record<string, number>,
    // the plan builds a value from these — required here, whatever the app's base view says
    required: { berater: true, faelligkeitsdatum: true, gesamtbetrag: true, kunde: true, mehrwertsteuer: true, nettobetrag: true, projekt: true, rechnungsdatum: true, zeiterfassungseintraege: true },
    initial: only(options.initial as FormValues | undefined, ["kunde", "projekt", "berater", "zeiterfassungseintraege", "rechnungsdatum", "faelligkeitsdatum", "rechnungsmonat", "rechnungsjahr", "nettobetrag", "mehrwertsteuer", "notizen", "gesamtbetrag"]),
    messages: only(options.messages as Record<string, string> | undefined, ["kunde", "projekt", "berater", "zeiterfassungseintraege", "rechnungsdatum", "faelligkeitsdatum", "rechnungsmonat", "rechnungsjahr", "nettobetrag", "mehrwertsteuer", "notizen", "gesamtbetrag"]),
  });
  ...
  const picks = {
    kunde: { ...searches.kunde, select: { ...searches.kunde.select, create: true as boolean } },
    projekt: { ...searches.projekt, select: { ...searches.projekt.select, create: false as boolean } },
    berater: { ...searches.berater, select: { ...searches.berater.select, create: false as boolean } },
    zeiterfassungseintraege: { ...searches.zeiterfassungseintraege, select: { ...searches.zeiterfassungseintraege.select, create: false as boolean } },
  };
