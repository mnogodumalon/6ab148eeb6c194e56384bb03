...
const DEFAULT_STEPS: Record<string, number> = {"berater": 3, "faelligkeitsdatum": 5, "kunde": 1, "mehrwertsteuer": 5, "nettobetrag": 5, "notizen": 5, "projekt": 2, "rechnungsdatum": 5, "rechnungsjahr": 5, "rechnungsmonat": 5, "zeiterfassungseintraege": 4};
...
export function useRechnungErstellenFlow(options: RechnungErstellenFlowOptions = {}) {
  ...
  const rechnungen = useStepForm('rechnungen', {
    fields: ["kunde", "projekt", "berater", "zeiterfassungseintraege", "rechnungsdatum", "faelligkeitsdatum", "rechnungsmonat", "rechnungsjahr", "nettobetrag", "mehrwertsteuer", "notizen"],
    steps: only(steps, ["kunde", "projekt", "berater", "zeiterfassungseintraege", "rechnungsdatum", "faelligkeitsdatum", "rechnungsmonat", "rechnungsjahr", "nettobetrag", "mehrwertsteuer", "notizen"]) as Record<string, number>,
    // the plan builds a value from these — required here, whatever the app's base view says
    required: { mehrwertsteuer: true, nettobetrag: true, rechnungsjahr: true },
    initial: only(options.initial as FormValues | undefined, ["kunde", "projekt", "berater", "zeiterfassungseintraege", "rechnungsdatum", "faelligkeitsdatum", "rechnungsmonat", "rechnungsjahr", "nettobetrag", "mehrwertsteuer", "notizen"]),
    messages: only(options.messages as Record<string, string> | undefined, ["kunde", "projekt", "berater", "zeiterfassungseintraege", "rechnungsdatum", "faelligkeitsdatum", "rechnungsmonat", "rechnungsjahr", "nettobetrag", "mehrwertsteuer", "notizen"]),
  });
  ...
  const picks = {
    ...
    projekt: { ...searches.projekt, select: { ...searches.projekt.select, create: true as boolean } },
