 ...
 *
 * Writes `rechnungen`: asks `projekt`, `berater`, `zeiterfassungseintraege`, `rechnungsmonat`, `rechnungsjahr`, `nettobetrag`, `mehrwertsteuer`, `faelligkeitsdatum`, `notizen`; sets `kunde`, `rechnungsdatum`, `rechnungsstatus` itself.
 ...
 *     steps: { projekt: 1, berater: 2, zeiterfassungseintraege: 3, rechnungsmonat: 4, rechnungsjahr: 4, nettobetrag: 4, mehrwertsteuer: 4, faelligkeitsdatum: 4, notizen: 4 },
...
export type RechnungErstellenFieldKey = 'berater' | 'faelligkeitsdatum' | 'mehrwertsteuer' | 'nettobetrag' | 'notizen' | 'projekt' | 'rechnungsjahr' | 'rechnungsmonat' | 'zeiterfassungseintraege';
...
const DEFAULT_STEPS: Record<string, number> = {"berater": 2, "faelligkeitsdatum": 4, "mehrwertsteuer": 4, "nettobetrag": 4, "notizen": 4, "projekt": 1, "rechnungsjahr": 4, "rechnungsmonat": 4, "zeiterfassungseintraege": 3};
...
export function useRechnungErstellenFlow(options: RechnungErstellenFlowOptions = {}) {
  ...
  const rechnungen = useStepForm('rechnungen', {
    fields: ["projekt", "berater", "zeiterfassungseintraege", "rechnungsmonat", "rechnungsjahr", "nettobetrag", "mehrwertsteuer", "faelligkeitsdatum", "notizen"],
    steps: only(steps, ["projekt", "berater", "zeiterfassungseintraege", "rechnungsmonat", "rechnungsjahr", "nettobetrag", "mehrwertsteuer", "faelligkeitsdatum", "notizen"]) as Record<string, number>,
    ...
    initial: only(options.initial as FormValues | undefined, ["projekt", "berater", "zeiterfassungseintraege", "rechnungsmonat", "rechnungsjahr", "nettobetrag", "mehrwertsteuer", "faelligkeitsdatum", "notizen"]),
    messages: only(options.messages as Record<string, string> | undefined, ["projekt", "berater", "zeiterfassungseintraege", "rechnungsmonat", "rechnungsjahr", "nettobetrag", "mehrwertsteuer", "faelligkeitsdatum", "notizen"]),
  });
  ...
  const plan: PlanStep[] = [
    {
      key: 'rechnungen', entity: 'rechnungen', form: rechnungen, primary: true,      values: (): FormValues => ({
        kunde: fromPick(picks.projekt, forms.rechnungen, "projekt", r => fieldRef(r, "kunde")),
