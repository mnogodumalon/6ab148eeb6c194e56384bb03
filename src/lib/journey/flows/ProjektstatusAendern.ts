 ...
 *
 * Writes `projekte`: asks `projektstatus`, `letzter_schritt`.
 ...
 *     steps: { projektstatus: 1, letzter_schritt: 1 },
 *   });
...
export type ProjektstatusAendernFieldKey = 'letzter_schritt' | 'projektstatus';
...
const DEFAULT_STEPS: Record<string, number> = {"letzter_schritt": 1, "projektstatus": 1};
export const PROJEKTSTATUSAENDERN_REVIEW_STEP = 2;
...
export function useProjektstatusAendernFlow(options: ProjektstatusAendernFlowOptions = {}) {
  ...
  return {
    ...
    reviewStep: PROJEKTSTATUSAENDERN_REVIEW_STEP,
