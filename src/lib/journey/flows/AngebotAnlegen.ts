/**
 * useAngebotAnlegenFlow — the plumbing of the flow « Angebot anlegen », generated from the plan.
 *
...
export type AngebotAnlegenFieldKey = 'angebotstyp' | 'berater' | 'beschreibung' | 'dauer' | 'kostenbetrag' | 'kostentyp' | 'projekt' | 'zeitrahmen_anfang' | 'zeitrahmen_ende';

export interface AngebotAnlegenForms {
  angebote: StepForm<'angebote'>;
}

// Alias so the option generics stay readable.
type Key = AngebotAnlegenFieldKey;

export interface AngebotAnlegenFlowOptions {
...
export const ANGEBOTANLEGEN_REVIEW_STEP = 4;
...
export function useAngebotAnlegenFlow(options: AngebotAnlegenFlowOptions = {}) {
  ...
  const forms: AngebotAnlegenForms = { angebote };
  ...
  const submit = useJourneySubmit(servicePort, plan, { draftKey: 'angebot-anlegen' });

  /** Props for a single-record pick step: {...flow.picks.x.select} {...flow.pick('x')} */
  const pick = (field: AngebotAnlegenFieldKey) => {
  ...
  const pickMany = (field: AngebotAnlegenFieldKey) => {
    const owner = formList.find(f => f.keys.includes(field)) ?? formList[0];
    const search = (picks as Record<string, { labelOf(id: string): string | undefined }>)[field];
  ...
  return {
    slug: 'angebot-anlegen' as const,
    draftKey: 'angebot-anlegen' as const,
    entity: 'angebote' as const,
    form: angebote,
    forms, formList, picks, submit, steps,    reviewStep: ANGEBOTANLEGEN_REVIEW_STEP,
    pick, pickMany, validateStep, reset,
  };
}

export type AngebotAnlegenFlow = ReturnType<typeof useAngebotAnlegenFlow>;
