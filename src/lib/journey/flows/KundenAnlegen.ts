/**
 * useKundenAnlegenFlow — the plumbing of the flow « Kunden anlegen », generated from the plan.
 *
 * Writes `kunden`: asks `kundenname`, `kundentyp`, `email`, `strasse`, `hausnummer`, `plz`, `ort`, `rechnungsadresse_gleich`, `rechnungsstrasse`, `rechnungshausnummer`, `rechnungsplz`, `rechnungsort`, `ansprechpartner_titel`, `ansprechpartner_vorname`, `ansprechpartner_nachname`, `ansprechpartner_email`, `bevorzugte_kontaktart`, `notizen`; sets `anlagedatum` itself.
 ...
 *     steps: { kundenname: 1, kundentyp: 1, email: 1, strasse: 1, hausnummer: 1, plz: 1, ort: 1, rechnungsadresse_gleich: 1, rechnungsstrasse: 1, rechnungshausnummer: 1, rechnungsplz: 1, rechnungsort: 1, ansprechpartner_titel: 1, ansprechpartner_vorname: 1, ansprechpartner_nachname: 1, ansprechpartner_email: 1, bevorzugte_kontaktart: 1, notizen: 1 },
 *   });
...
export type KundenAnlegenFieldKey = 'ansprechpartner_email' | 'ansprechpartner_nachname' | 'ansprechpartner_titel' | 'ansprechpartner_vorname' | 'bevorzugte_kontaktart' | 'email' | 'hausnummer' | 'kundenname' | 'kundentyp' | 'notizen' | 'ort' | 'plz' | 'rechnungsadresse_gleich' | 'rechnungshausnummer' | 'rechnungsort' | 'rechnungsplz' | 'rechnungsstrasse' | 'strasse';

export interface KundenAnlegenForms {
  kunden: StepForm<'kunden'>;
}

// Alias so the option generics stay readable.
type Key = KundenAnlegenFieldKey;

export interface KundenAnlegenFlowOptions {
...
const DEFAULT_STEPS: Record<string, number> = {"ansprechpartner_email": 1, "ansprechpartner_nachname": 1, "ansprechpartner_titel": 1, "ansprechpartner_vorname": 1, "bevorzugte_kontaktart": 1, "email": 1, "hausnummer": 1, "kundenname": 1, "kundentyp": 1, "notizen": 1, "ort": 1, "plz": 1, "rechnungsadresse_gleich": 1, "rechnungshausnummer": 1, "rechnungsort": 1, "rechnungsplz": 1, "rechnungsstrasse": 1, "strasse": 1};
export const KUNDENANLEGEN_REVIEW_STEP = 2;
...
export function useKundenAnlegenFlow(options: KundenAnlegenFlowOptions = {}) {
  const steps = { ...DEFAULT_STEPS, ...(options.steps ?? {}) } as Record<string, number>;
  const kunden = useStepForm('kunden', {
    fields: ["kundenname", "kundentyp", "email", "strasse", "hausnummer", "plz", "ort", "rechnungsadresse_gleich", "rechnungsstrasse", "rechnungshausnummer", "rechnungsplz", "rechnungsort", "ansprechpartner_titel", "ansprechpartner_vorname", "ansprechpartner_nachname", "ansprechpartner_email", "bevorzugte_kontaktart", "notizen"],
    steps: only(steps, ["kundenname", "kundentyp", "email", "strasse", "hausnummer", "plz", "ort", "rechnungsadresse_gleich", "rechnungsstrasse", "rechnungshausnummer", "rechnungsplz", "rechnungsort", "ansprechpartner_titel", "ansprechpartner_vorname", "ansprechpartner_nachname", "ansprechpartner_email", "bevorzugte_kontaktart", "notizen"]) as Record<string, number>,
    initial: only(options.initial as FormValues | undefined, ["kundenname", "kundentyp", "email", "strasse", "hausnummer", "plz", "ort", "rechnungsadresse_gleich", "rechnungsstrasse", "rechnungshausnummer", "rechnungsplz", "rechnungsort", "ansprechpartner_titel", "ansprechpartner_vorname", "ansprechpartner_nachname", "ansprechpartner_email", "bevorzugte_kontaktart", "notizen"]),
    messages: only(options.messages as Record<string, string> | undefined, ["kundenname", "kundentyp", "email", "strasse", "hausnummer", "plz", "ort", "rechnungsadresse_gleich", "rechnungsstrasse", "rechnungshausnummer", "rechnungsplz", "rechnungsort", "ansprechpartner_titel", "ansprechpartner_vorname", "ansprechpartner_nachname", "ansprechpartner_email", "bevorzugte_kontaktart", "notizen"]),
  });
  const forms: KundenAnlegenForms = { kunden };
  ...
  const submit = useJourneySubmit(servicePort, plan, { draftKey: 'kunden-anlegen' });

  /** Props for a single-record pick step: {...flow.picks.x.select} {...flow.pick('x')} */
  const pick = (field: KundenAnlegenFieldKey) => {
  ...
  const pickMany = (field: KundenAnlegenFieldKey) => {
    const owner = formList.find(f => f.keys.includes(field)) ?? formList[0];
    const search = (picks as Record<string, { labelOf(id: string): string | undefined }>)[field];
  ...
  return {
    slug: 'kunden-anlegen' as const,
    draftKey: 'kunden-anlegen' as const,
    entity: 'kunden' as const,
    form: kunden,
    forms, formList, picks, submit, steps,    reviewStep: KUNDENANLEGEN_REVIEW_STEP,
    pick, pickMany, validateStep, reset,
  };
}

export type KundenAnlegenFlow = ReturnType<typeof useKundenAnlegenFlow>;
