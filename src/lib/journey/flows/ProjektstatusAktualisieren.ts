/**
 * useProjektstatusAktualisierenFlow — the plumbing of the flow « Projektstatus aktualisieren », generated from the plan.
 *
...
export type ProjektstatusAktualisierenFieldKey = 'letzter_schritt' | 'projekte' | 'projektstatus';

export interface ProjektstatusAktualisierenForms {
  projekte: StepForm<'projekte'>;
}

// Alias so the option generics stay readable.
type Key = ProjektstatusAktualisierenFieldKey;

export interface ProjektstatusAktualisierenFlowOptions {
...
export const PROJEKTSTATUSAKTUALISIEREN_REVIEW_STEP = 3;
...
export function useProjektstatusAktualisierenFlow(options: ProjektstatusAktualisierenFlowOptions = {}) {
  ...
  const forms: ProjektstatusAktualisierenForms = { projekte };
  ...
  const submit = useJourneySubmit(servicePort, plan, { draftKey: 'projektstatus-aktualisieren' });
  ...
  const pick = (field: ProjektstatusAktualisierenFieldKey) => {
    if (field in targets) {
  ...
  const pickMany = (field: ProjektstatusAktualisierenFieldKey) => {
    const owner = formList.find(f => f.keys.includes(field)) ?? formList[0];
    const search = (picks as Record<string, { labelOf(id: string): string | undefined }>)[field];
  ...
  return {
    slug: 'projektstatus-aktualisieren' as const,
    draftKey: 'projektstatus-aktualisieren' as const,
    entity: 'projekte' as const,
    form: projekte,
    forms, formList, picks, submit, steps, targets,    reviewStep: PROJEKTSTATUSAKTUALISIEREN_REVIEW_STEP,
    pick, pickMany, validateStep, reset,
  };
}

export type ProjektstatusAktualisierenFlow = ReturnType<typeof useProjektstatusAktualisierenFlow>;
