/**
 * useStundenErfassenFlow — the plumbing of the flow « Stunden erfassen », generated from the plan.
 *
 * Writes `zeiterfassung`: asks `berater`, `projekt`, `datum`, `stunden`, `leistung`, `taetigkeit`, `erfassungsmonat`, `erfassungsjahr`, `abrechenbar`.
...
export type StundenErfassenFieldKey = 'abrechenbar' | 'berater' | 'datum' | 'erfassungsjahr' | 'erfassungsmonat' | 'leistung' | 'projekt' | 'stunden' | 'taetigkeit';

export interface StundenErfassenForms {
  zeiterfassung: StepForm<'zeiterfassung'>;
}

// Alias so the option generics stay readable.
type Key = StundenErfassenFieldKey;

export interface StundenErfassenFlowOptions {
...
const DEFAULT_STEPS: Record<string, number> = {"abrechenbar": 4, "berater": 1, "datum": 4, "erfassungsjahr": 4, "erfassungsmonat": 4, "leistung": 3, "projekt": 2, "stunden": 4, "taetigkeit": 4};
export const STUNDENERFASSEN_REVIEW_STEP = 5;
...
export function useStundenErfassenFlow(options: StundenErfassenFlowOptions = {}) {
  const steps = { ...DEFAULT_STEPS, ...(options.steps ?? {}) } as Record<string, number>;
  const zeiterfassung = useStepForm('zeiterfassung', {
    fields: ["berater", "projekt", "datum", "stunden", "leistung", "taetigkeit", "erfassungsmonat", "erfassungsjahr", "abrechenbar"],
    steps: only(steps, ["berater", "projekt", "datum", "stunden", "leistung", "taetigkeit", "erfassungsmonat", "erfassungsjahr", "abrechenbar"]) as Record<string, number>,
    initial: only(options.initial as FormValues | undefined, ["berater", "projekt", "datum", "stunden", "leistung", "taetigkeit", "erfassungsmonat", "erfassungsjahr", "abrechenbar"]),
    messages: only(options.messages as Record<string, string> | undefined, ["berater", "projekt", "datum", "stunden", "leistung", "taetigkeit", "erfassungsmonat", "erfassungsjahr", "abrechenbar"]),
  });
  const forms: StundenErfassenForms = { zeiterfassung };
  ...
  const submit = useJourneySubmit(servicePort, plan, { draftKey: 'stunden-erfassen' });

  /** Props for a single-record pick step: {...flow.picks.x.select} {...flow.pick('x')} */
  const pick = (field: StundenErfassenFieldKey) => {
  ...
  const pickMany = (field: StundenErfassenFieldKey) => {
    const owner = formList.find(f => f.keys.includes(field)) ?? formList[0];
    const search = (picks as Record<string, { labelOf(id: string): string | undefined }>)[field];
  ...
  return {
    slug: 'stunden-erfassen' as const,
    draftKey: 'stunden-erfassen' as const,
    entity: 'zeiterfassung' as const,
    form: zeiterfassung,
    forms, formList, picks, submit, steps,    reviewStep: STUNDENERFASSEN_REVIEW_STEP,
    pick, pickMany, validateStep, reset,
  };
}

export type StundenErfassenFlow = ReturnType<typeof useStundenErfassenFlow>;
