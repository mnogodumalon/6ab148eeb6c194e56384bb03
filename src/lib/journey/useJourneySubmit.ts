...
import { checkPlanWrite, flowSlugFromLocation } from './planGuard';
...
async function createFromStep(step: PlanStep, ctx: PlanContext): Promise<JourneyRecord> {
  ...
  // The plan's Schreibliste, enforced (docs/orchestrator/SPEC.md §6.1): a
  // write outside it never leaves the browser. No plan → nothing refused.
  const refused = checkPlanWrite(flowSlugFromLocation(), step.entity, payload);
  if (refused) throw new JourneyPortError(refused);
