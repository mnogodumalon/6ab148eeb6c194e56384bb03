...
//   ERROR  the plan orders exports but nothing calls downloadCsv
//
...
// ── Exports: a planned download needs a downloadCsv call somewhere in src.
// ERROR, not WARN: unlike a missing flow page there IS a file to repair — the
// overview — and a plan that orders a file the build never produces is the
// exact failure this sort was introduced for.
const exports_ = Array.isArray(plan.exports) ? plan.exports : [];
if (exports_.length) {
  const walk = (dir) => {
    let out = [];
    for (const e of readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (/\.tsx?$/.test(e.name) && full !== join('src', 'lib', 'export.ts')) out.push(full);
    }
    return out;
  };
  let calls = 0;
  for (const file of existsSync('src') ? walk('src') : []) {
    calls += (readFileSync(file, 'utf8').match(/\bdownloadCsv\s*\(/g) || []).length;
  }
  if (calls < exports_.length) {
    const names = exports_.map(e => e.id).join(', ');
    errors.push(`src/pages/DashboardOverview.tsx: the plan orders ${exports_.length} export(s) (${names}) `
      + `but src calls downloadCsv ${calls} time(s) — wire one download button per planned export `
...
console.log(`check-plan: OK (${flows.length} flows, ${pages.length} public pages, ${exports_.length} exports held against the plan)`);
